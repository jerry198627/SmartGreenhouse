...
def handle_client(conn, addr):          #Socket(Rx/Tx), UART(Tx)
    global connected, Settings_dict, Update_dict
    ...
    while connected:
        ...
        if msg:
            ...
            if msg == "GetUpdate":
                ...
                print(f"[Conn: {addr}] Setting List : {Settings_dict} Get: {tempmsg}")
                print(type(tempmsg))
                if type(tempmsg) == type(Settings_dict):
                    if tempmsg != Settings_dict:
                        print(f"[Conn:{addr}] Settinglist Matched \n")
                ...
                '''
                print(f"[Conn: {addr}] Update_dict : {Update_dict}")
